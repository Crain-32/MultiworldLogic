...
from extras import item_id_dict, capital_case_with_space, chart_macro_to_island
from gameitem import *
from logic.world import World


def generate_spoiler_log(worlds: list[World]):
    with open("spoilerlog.txt", 'w') as spoiler_log_file:
        for world in worlds:
            spoiler_log_file.write(world.world_settings.spoiler_representation(world.world_id))
            if world.world_settings.race_mode:
                spoiler_log_file.write(f"Race Mode Dungeons for World {world.world_id}\n")
        ...
        for world_id, required_items_in_world in enumerate(required_items):
            spoiler_log_file.write(f"World {world_id} Potentially Major Items\n")
            ...
            for item_type, location_list in item_type_groups:
                ...
                for location in location_list:
                    if item_type == ITEM_IS_TRIFORCE_CHART or item_type == ITEM_IS_TREASURE_CHART:
                        chart_locations[location.current_item.world_id][location.current_item.game_item_id] = location.short_hand_loc_rep()
                    if "SunkenTreasure" in location.name:
                        sunken_treasures[location.world_id][capital_case_with_space(location.name)] = location.short_hand_item_rep()
                    spoiler_log_file.write(f"{item_id_to_name_dict[location.current_item.game_item_id]}: {location.short_hand_loc_rep()}\n")
                spoiler_log_file.write("\n")
        spoiler_log_file.write("\n")

        for world_id, world in enumerate(worlds):
            spoiler_log_file.write(f"Sunken Treasure and Charts for World {world_id}\n")
